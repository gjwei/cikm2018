...
from models.utils.Embedding import Embedding
...
class ESIM(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.project_input = config.project_input
        self.embedding_size = config.embed_size
        self.distance_biases = config.distance_biases
        self.max_sentence_length = config.max_sentence_length
        self.pretrained_emb = config.pretrained_emb
        
        self.embed_dropout = nn.Dropout(config.embed_dropout)
        
        self.dropout = nn.Dropout(p=self.config.dropout)
        
        self.word_embedding = nn.Embedding(self.vocab_size, self.embedding_size, padding_idx=config.pad_index)

        self.pos_embedding = nn.Embedding(self.max_sentence_length + 1, self.embedding_size, padding_idx=config.pad_index)

        # torch.nn.init.uniform_(self.pos_embedding.weight, -0.02, 0.02)
        
        self.lstm_intra = LSTM(torch.cuda.is_available(), self.embedding_size, self.num_units)
        # self.lstm_intra = nn.LSTM(self.embedding_size, self.num_units)
        
        self.linear_layer_compare = nn.Sequential(nn.Linear(4 * self.num_units * 2, self.num_units), nn.ELU(),
                                                  nn.Dropout(p=self.config.dropout))
        
        self.lstm_compare = LSTM(torch.cuda.is_available(), self.embedding_size, self.num_units)
        # self.lstm_compare = nn.LSTM(self.embedding_size, self.num_units)
        
        self.linear_layer_aggregate = nn.Sequential(nn.BatchNorm1d(self.num_units * 8 + config.extra_feature_nums),
                                                    nn.Dropout(p=self.config.dropout),
                                                    nn.Linear(4 * self.num_units * 2 + config.extra_feature_nums,
    ...
    def initialize_lstm(self):
        if torch.cuda.is_available():
            init = torch.Tensor(
                    np.concatenate([self.ortho_weight(), self.ortho_weight(), self.ortho_weight(), self.ortho_weight()],
                                   0)).cuda()
        else:
            init = torch.Tensor(
                    np.concatenate([self.ortho_weight(), self.ortho_weight(), self.ortho_weight(), self.ortho_weight()],
    ...
    def init_weight(self):
        ...
        self.linear_layer_aggregate[2].weight.data.normal_(0, 0.01)
        self.linear_layer_aggregate[2].bias.data.fill_(0)
        self.linear_layer_aggregate[6].weight.data.normal_(0, 0.01)
        self.linear_layer_aggregate[6].bias.data.fill_(0)
        # ipdb.set_trace()
        self.word_embedding.weight.data.copy_(torch.from_numpy(self.pretrained_emb))
        # fix embedding layers
        self.word_embedding.weight.requires_grad = False

        self.pos_embedding.weight.data.copy_(position_encoding_init(self.max_sentence_length + 1, self.embedding_size))
    ...
    def aggregate(self, v1, v2, extra=None):
        """
        ...
        v = torch.cat((v1_mean, v1_max, v2_mean, v2_max), 1)
        if self.config.use_extra:
            v = torch.cat((v, extra), dim=1)
        out = self.linear_layer_aggregate(v)
    ...
    def get_pos(self, x):
        ...
        # ipdb.set_trace()
        result = torch.cat([torch.arange(1, self.max_sentence_length + 1).view(-1, 1) for _ in range(x.size(1))],dim=1)
        pad_index = x.eq(self.config.pad_index)
        result[pad_index] = self.config.pad_index
        return result.long().cuda()

    def forward(self, x1, x1_mask, x2, x2_mask, extra=None):
        # idx = [i for i in range(embed_sent.size(1) - 1, -1, -1)]
        # if torch.cuda.is_available():
        #	idx = torch.cuda.LongTensor(idx)
        # else:
        #	idx = torch.LongTensor(idx)
        # 原始输入是[batch, length], 要transpose为{length, batch]
        x1 = torch.transpose(x1, 0, 1)
        x2 = torch.transpose(x2, 0, 1)
        x1_mask = torch.transpose(x1_mask, 0, 1)
        x2_mask = torch.transpose(x2_mask, 0, 1)

        # x1_pos = self.get_pos(x1)
        # x2_pos = self.get_pos(x2)

        x1 = self.word_embedding(x1)
        x1 = self.embed_dropout(x1)
        x2 = self.word_embedding(x2)

        x2 = self.embed_dropout(x2)
        ...
        logits = self.aggregate(v1, v2, extra)
        return logits
...
class ESIM_WithCharEmbedding(nn.Module):
    def __init__(self, config):
        """
        Create the model based on MLP networks.
        :param num_units: size of the networks
        ...
        :param distance_biases: number of different distances with biases used
            in the intra-attention model
        """
        super(ESIM_WithCharEmbedding, self).__init__()
        self.config = config
        
        self.vocab_size = config.vocab_size
        self.num_units = config.char_num_units
        self.num_classes = config.num_classes
        self.project_input = config.project_input
        self.embedding_size = config.embed_size
        self.distance_biases = config.distance_biases
        ...
        # ipdb.set_trace()
        self.lstm_intra = LSTM(torch.cuda.is_available(),
                               self.embedding_size + self.config.char_output_dim,
                               self.num_units)
        
        self.linear_layer_compare = nn.Sequential(nn.Linear(4 * self.num_units * 2, self.num_units),
                                                  nn.ELU(),
                                                  nn.Dropout(self.config.dropout))
        
        self.lstm_compare = LSTM(torch.cuda.is_available(),
                                 self.embedding_size + self.config.char_output_dim ,
                                 self.num_units)
        
        self.linear_layer_aggregate = nn.Sequential(nn.Dropout(self.config.dropout),
                                                    nn.Linear(4 * self.num_units * 2 + config.extra_feature_nums,
                                                              self.config.hidden_size),
                                                    nn.ELU(),
                                                    nn.Dropout(self.config.dropout),
    ...
    def forward(self, x1, x1_mask, x2, x2_mask, x1_chars, x2_chars, extra=None):
        # idx = [i for i in range(embed_sent.size(1) - 1, -1, -1)]
        # if torch.cuda.is_available():
        #	idx = torch.cuda.LongTensor(idx)
        # else:
        #	idx = torch.LongTensor(idx)
        # 原始输入是[batch, length], 要transpose为{length, batch]
        x1 = self.embedding(x1, x1_chars)

        # x2 = self.word_embedding(x2)
        x2 = self.embedding(x2, x2_chars)
